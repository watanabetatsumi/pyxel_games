...
class App:
    ...
    def update(self):
        ...
        if pyxel.btnp(pyxel.KEY_SPACE): 
            self.bullets.append(Bullet()) # self.bulletsリストにappendを用いて追加 
        for bullet in self.bullets:
            bullet.move(self.balls,self.count)
...
class Bullet:
    def __init__(self):
        self.y = 200  # 初期化した時には弾が見えないように最上段に置く。
        self.pdx = pyxel.mouse_x#オブジェクトが生成された瞬間のx座標を取り込む

    def move(self,balls,point):
        self.y -= 5
        for ball in balls:
            if math.sqrt((self.pdx - ball.x) ** 2 + (self.y - ball.y) **2) <= 12:
                self.y=0
                ball.restart()
                point.plus()
                pyxel.play(0,1)
